...
def submit_escalation():
    try:
        ...
        escalation_type = data.get('escalationType', 'Standard Request')
        
        # Convert escalation type to severity number
        severity_map = {
            'SEV1': '1',
            'SEV2': '2', 
            'Exception Request': '3',
        ...
        claimed_severity = severity_map.get(escalation_type, '4')
        ...
        if actual_severity and claimed_severity != actual_severity:
            ...
            # Post severity mismatch alert to tracking channel
            ...
            # Send coaching message to submitter
            submitter_alias = data.get('firstApprover', '').replace('@', '')  # Use first approver as submitter for now
            coaching_msg = f"⚠️ Severity Mismatch Notice: You claimed {escalation_type} but the SIM ticket shows SEV-{actual_severity}. Please ensure severity claims match actual ticket severity for future escalations."
            send_slack_message(submitter_alias, coaching_msg)
            
            # Alert DFA about false escalation
            dfa_msg = f"🚨 False Escalation Alert: {submitter_alias} submitted {escalation_type} escalation but SIM ticket {sim_link} shows SEV-{actual_severity}. Coaching message sent to submitter."
            send_slack_message(data['firstApprover'], dfa_msg)
