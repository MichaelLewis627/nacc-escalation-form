...
def submit_escalation():
    try:
        ...
        if actual_severity and claimed_severity != actual_severity:
            ...
            coaching_msg = f"⚠️ Severity Mismatch Notice: You claimed SEV-{claimed_severity} but the SIM ticket shows SEV-{actual_severity}. Please ensure severity claims match actual ticket severity for future escalations."
            send_slack_message(data.get('submitterAlias', ''), coaching_msg)
            
            # Alert DFA about false escalation
            dfa_msg = f"🚨 False Escalation Alert: {data.get('submitterAlias', 'Unknown')} submitted SEV-{claimed_severity} escalation but SIM ticket {sim_link} shows SEV-{actual_severity}. Coaching message sent to submitter."
