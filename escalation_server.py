...
def validate_sev_classification(submission_data):
    ...
    # Use client-side SIM lookup if available, otherwise try server lookup
    client_sim_sev = submission_data.get('simTicketSev', '').strip()
    
    if client_sim_sev:
        # Client successfully looked up the severity
        has_sim_ticket = True
        actual_sim_sev = client_sim_sev
        ticket_id = extract_sim_ticket_id(sim_link)
    else:
        # Fallback to server lookup (will likely fail on Railway)
        ticket_id = extract_sim_ticket_id(sim_link)
    ...
    if is_high_sev:
        if not has_sim_ticket:
            actual_sev = 'SEV3'  # Changed from 'Standard' to 'SEV3'
    ...
    # Check need by date for urgency validation
...
def check_repeat_escalator(alias):
    """Check if user is a repeat false escalator"""
    if not s3_client:
        ...
        for line in lines:
            if line.strip():
                ...
                if len(parts) >= 3:
                    ...
                    user_alias = parts[2].strip('"')  # alias column
                    false_esc = parts[14].strip('"') if len(parts) > 14 else 'False'
                    
                    try:
                        log_date = datetime.fromisoformat(timestamp_str.replace('Z', '+00:00'))
                        if (user_alias == alias and 
                            log_date > cutoff_date and 
                            false_esc.lower() == 'true'):
                            false_count += 1
                    except:
                        continue
        
        return {
            'is_repeat': false_count >= 2,
            'count': false_count
        }
    except:
        return {'is_repeat': False, 'count': 0}
...
def log_to_s3(submission_data):
    ...
    try:
        ...
        repeat_info = check_repeat_escalator(submission_data.get('alias', ''))
        
        # Send behavior notifications
        send_behavior_notification(submission_data, validation_result, repeat_info)
        ...
        new_header = "timestamp,submission_id,alias,claimed_sev,actual_sev,sim_ticket_sev,station,coupa_link,sim_link,sim_ticket_id,need_by_date,description,first_approver,second_approver,false_escalation,mismatch_reason,has_sim_ticket,repeat_count,source\n"
        
        if not file_exists or not existing_content:
            csv_content = new_header
        else:
