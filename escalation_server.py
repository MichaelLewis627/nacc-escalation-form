...
def check_repeat_escalator(first_approver):
        ...
        for line in lines:
            if line.strip():
                ...
                if len(parts) >= 12:
                    ...
                    user = parts[11].strip('"')  # first_approver column
                    false_esc = parts[13].strip('"') if len(parts) > 13 else 'False'
                    
                    try:
                        log_date = datetime.fromisoformat(timestamp_str.replace('Z', '+00:00'))
                        if (user == first_approver and 
...
def log_to_s3(submission_data):
    ...
    try:
        ...
        repeat_info = check_repeat_escalator(submission_data.get('firstApprover', ''))
